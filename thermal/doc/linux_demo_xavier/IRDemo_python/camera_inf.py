...
if WIDTH == 256 and HEIGHT == 192:
    DATA_HEIGHT = HEIGHT*2  # k12e2时，DATA_HEIGHT是HEIGHT的2倍，其它型号两者相同
else:
    DATA_HEIGHT = HEIGHT*1  # 其它型号两者相同
...
class Frame(Structure):
    _fields_ = [('width', c_uint16),
                ...
                ('timems', c_uint32),
                ('nouse4', c_uint8 * 4),
                ('buffer', c_uint16 * WIDTH * DATA_HEIGHT)]
...
def sdk_creat_connect(handle, ipaddr, callbackfun, this):
    data = py_object(this)
    ret = dll.IRSDK_Create(handle, ipaddr, callbackfun, None, None, data)
    ret = dll.IRSDK_Connect(handle)
...
def sdk_frame2gray(frame, gray):
    ...
    dll.IRSDK_Frame2GrayDDE(frame, gray, 50, 50 , 0, 0,0,0,0)
...
def sdk_saveframe2jpg(path, frame, rgb):
    ret = dll.IRSDK_SaveFrame2Jpeg(path, frame, rgb, 0, None)
    return ret
...
def get_pt_temp(frame, x, y):
    array = np.array(frame.buffer)
    temp_array = array[:frame.height, :frame.width]
    temp = (temp_array[y, x] - 10000) / frame.TempDiv
    return temp


# 获取点温度
def get_pt_temp_fbuf(buffer, x, y, d):
    temp = (buffer[y, x] - 10000) / d
    return temp
