...
def convert_rg8_to_rgb(input_folder, output_folder):
    ...
    for raw_file in sorted(raw_files):
        ...
        with open(raw_file, 'rb') as f:
            # 读取文件头信息
            num_images, offset_x, offset_y, width, height = np.fromfile(f, dtype=np.int32, count=5)
            ...
            for i in range(num_images):
                # 读取单张图像数据
                bayer_data = np.fromfile(f, dtype=np.uint8, count=width*height).reshape((height, width))
                
                # 转换为RGB
                rgb_image = cv.cvtColor(bayer_data, bayer_pattern)
                
                # 保存转换后的图像
                output_path = os.path.join(output_folder, f"{base_name}_{i:03d}.png")
                cv.imwrite(output_path, cv.cvtColor(rgb_image, cv.COLOR_RGB2BGR))
                
                print(f"已转换: {base_name}_{i:03d}")
