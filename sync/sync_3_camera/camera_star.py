...
class CameraStar:
    def __init__(self):
        ...
        # 初始化帧处理
        self.glbFrame = Frame()
        self.sizeFrame = 32 + THERMAL_WIDTH * THERMAL_HEIGHT * 2
    ...
    def connect_camera(self, ip_address, port=None):
        """连接相机"""
        if port is None:
            port = THERMAL_CAMERA_PORT
            if port is None:  # 如果配置文件中也未指定端口
...
def main():
    ...
    result = camera.connect_camera(THERMAL_CAMERA_IP,THERMAL_CAMERA_PORT)
    if result:
        print("相机连接成功")
    else:   
        print("相机连接失败")
    ...
    camera.set_temp_segment(THERMAL_TEMP_SEGMENT)
    camera.calibration()
